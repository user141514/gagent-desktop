...
When scoring the default latest reports, passed optional E2E `ledger_summary` values must also match the raw runtime ledger JSONL for the same run id under `backend/runtime_ledger/runs`; isolated `--results-dir --no-write` scoring intentionally skips this raw-ledger binding.
