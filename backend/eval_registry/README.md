...
Strict/full convergence also requires `evidence.e2e_env.GAGENT_E2E_DEPS` to point at this checkout's `backend/temp/e2e_deps`, that directory to contain the `agents`, `browser_use`, and `playwright` import markers, and both opt-in E2E switches to be `1`.
