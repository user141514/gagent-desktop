...
Internal eval reports must include every case from `backend/eval_registry/cases/*.json`; `results` must be a list of objects, and each result must have a `total` between 0 and 100 plus a `verdict` of `pass`, `fail`, or `skip`; missing, unexpected, duplicate case ids, non-object result entries, or summary counts/status that do not match the results invalidate the internal eval component.
