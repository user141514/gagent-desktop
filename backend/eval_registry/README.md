...
Each eval case `expected_tools.allowed` list must include its `target_tool`; `expected_tools.allowed` and `expected_tools.forbidden` must be disjoint; every referenced tool must exist in `backend/tool_registry/tools/*.yml`; `expected_result` may only contain validator-known fields; `expected_result.allow_success` and `expected_result.allow_structured_failure` must be booleans with at least one true; tool-specific `expected_result` fields must match the case `target_tool` and `type`; `expected_result.require_final_status` must match the ledger summary when present; `expected_result.require_runtime_events` must use supported RuntimeHost event types; `expected_result.require_balanced_turn_events` requires both `llm_call_started` and `llm_call_completed`; `expected_ledger.required_events` and `expected_ledger.required_on_failure` must use supported `runtime_ledger` event types; `expected_ledger.required_decision_forbidden_actions` must stay within `expected_tools.forbidden`. The validator treats mismatches as contract drift.
