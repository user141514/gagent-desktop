...
Passed optional E2E reports must also use known top-level fields and `ledger_summary` fields from `runtime_ledger.RUNTIME_LEDGER_SUMMARY_FIELDS`, include a smoke-typed run id (`openai_e2e_` for OpenAI and `browser_agent_e2e_` for browser_agent) and successful runtime_ledger evidence; OpenAI reports must include the expected sentinel, an empty `ledger_summary.tools`, plus RuntimeHost observability fields/alignment from runtime_ledger, and browser_agent reports must include a successful tool result using `core.browser_agent.BROWSER_AGENT_RESULT_FIELDS` with non-empty output, positive step count, `ledger_summary.tools` containing only positive `browser_agent` evidence, and `ledger_event_count` matching `ledger_summary.event_count`.
