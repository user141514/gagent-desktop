...
Each eval case `type` and `version` must be supported by the validator; top-level id/type/task/owner_layer/target_tool values must be non-empty JSON strings, and version must be a JSON integer; top-level object plus `input`, `expected_tools`, `expected_ledger`, `expected_result`, and `score` may only contain registry/validator-known fields; `score` weights must be JSON integers and match the current 60/40 harness contract; `input` fields are scoped by `type` and `target_tool`; required `input` fields must be present; `input` text fields must be non-empty strings, boolean switches must be booleans, and numeric limit fields must be positive numbers/integers; `expected_tools.allowed` list must include its `target_tool`; `expected_tools.allowed` and `expected_tools.forbidden` must be disjoint, contain no duplicates, and contain only strings; every referenced tool must exist in `backend/tool_registry/tools/*.yml`; `expected_result` list fields must be lists, contain only strings, and contain no duplicates; `expected_result` boolean switches must be booleans; `expected_result.allow_success` and `expected_result.allow_structured_failure` must include at least one true; tool-specific `expected_result` fields must match the case `target_tool` and `type`; `expected_result.require_final_status` must match the ledger summary when present; `expected_result.require_runtime_events` must use supported RuntimeHost event types; `expected_result.require_balanced_turn_events` requires both `llm_call_started` and `llm_call_completed`; `expected_ledger` list fields must be lists, contain only strings, and contain no duplicates; `expected_ledger.required_events` and `expected_ledger.required_on_failure` must use supported `runtime_ledger` event types; `expected_ledger.required_decision_forbidden_actions` must stay within `expected_tools.forbidden`. The validator treats mismatches as contract drift.
...
Score reports include an `evidence` object with the results directory, input report file status, optional E2E raw ledger file status, Git HEAD/dirty state, Python executable, and non-secret E2E env switches.
Internal eval reports must use `run_eval_cases.EVAL_REPORT_FIELDS`, `run_eval_cases.EVAL_RESULT_FIELDS`, `run_eval_cases.EVAL_FINAL_ANSWER_FIELDS`, and the eval list-field sets, include every case from `backend/eval_registry/cases/*.json`; `results` must be a list of objects, and each result must have a `total` between 0 and 100 plus a `verdict` of `pass`, `fail`, or `skip`; unknown fields, invalid list evidence fields, unknown runtime_ledger observability fields, missing, unexpected, duplicate case ids, non-object result entries, or summary counts/status that do not match the results invalidate the internal eval component.
...
Skipped/failed optional OpenAI/browser_agent E2E reports count as `needs_work` and must use known top-level fields; full completion requires enabling the opt-in E2E env vars before running `--refresh --strict`.
Passed optional E2E reports must also use known top-level fields and `ledger_summary` fields from `runtime_ledger.RUNTIME_LEDGER_SUMMARY_FIELDS`, include a smoke-typed run id (`openai_e2e_` for OpenAI and `browser_agent_e2e_` for browser_agent), non-empty task, matching owner layer, and successful runtime_ledger evidence; OpenAI reports must include the expected sentinel in both task/result, `Layer 3 runtime controller`, an empty `ledger_summary.tools`, plus RuntimeHost observability fields/alignment from runtime_ledger, and browser_agent reports must include `Layer 1 capability contract`, a successful tool result using `core.browser_agent.BROWSER_AGENT_RESULT_FIELDS` with non-empty output, positive step count, `ledger_summary.tools` containing only positive `browser_agent` evidence, and `ledger_event_count` matching `ledger_summary.event_count`.
When scoring the default latest reports, passed optional E2E `ledger_summary` values and event sequences must also match the raw runtime ledger JSONL for the same run id under `backend/runtime_ledger/runs`; OpenAI raw events must be `run_started -> run_finished`, browser_agent raw events must be `run_started -> tool_call -> tool_result -> run_finished`, and isolated `--results-dir --no-write` scoring intentionally skips this raw-ledger binding.
`run_convergence_checks.py` validates the score JSON, runner mode flags, expected component names/weights/status fields, component field whitelists, total/max_total/status/blockers consistency, required `evidence` fields, evidence subobject field whitelists, current Git HEAD/branch/dirty alignment, bundled Python/results-directory alignment, and non-empty input report evidence, then cross-checks refreshed input report and passed-E2E raw ledger file bytes/timestamps against the files on disk, verifies `latest_functionality_score.json` matches score stdout, and prints the score on success so baseline runs expose current blockers instead of only saying `ok`; `--full` also runs the baseline validators before strict scoring.
Refreshed score output must show input report modification timestamps close to `generated_at_utc`; stale or future report evidence is rejected.
Strict/full convergence also requires the runner-confirmed current checkout dirty state to be false; score evidence cannot pass by merely claiming `evidence.source_git.dirty` is false.
Strict/full convergence also requires `evidence.e2e_env.GAGENT_E2E_DEPS` to point at this checkout's `backend/temp/e2e_deps`, that directory to contain the `agents`, `browser_use`, and `playwright` import markers plus the pinned `.dist-info` directories from `backend/requirements-e2e.txt`, each pinned `.dist-info/METADATA` to report the expected Name/Version, and both opt-in E2E switches to be `1`.
