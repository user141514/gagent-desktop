...
Passed optional E2E reports must also use known top-level fields and `ledger_summary` fields from `runtime_ledger.RUNTIME_LEDGER_SUMMARY_FIELDS`, include run id and successful runtime_ledger evidence; OpenAI reports must include the expected sentinel plus RuntimeHost observability fields/alignment from runtime_ledger, and browser_agent reports must include a successful tool result using `core.browser_agent.BROWSER_AGENT_RESULT_FIELDS` with non-empty output and positive step count.
