...
## Learnings
- 2026-05-03: 收尾三件事缺一不可：写报告 → `complete_task(tasktitle, historyline, report_path)` → `set_todo()` 标记完成。跳过任何一步都会导致任务状态不一致
- 2026-05-03: 任务完成后必须触发记忆蒸馏：检查是否有新的环境事实/用户偏好/重要步骤值得写入 memory/；有则用 `file_patch` 最小化更新对应 SOP 的 Learnings 章节
- 2026-05-03: 连续两次不选相同子任务，避免陷入局部循环；每次执行前先 `get_history(40)` 了解上下文避免重复工作
- 2026-05-03: 即使实验失败也要记录过程和结果，失败报告同样有价值——记录"尝试了什么、为什么失败、排除了什么方向"