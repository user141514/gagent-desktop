...
- full convergence runner rejects strict score evidence unless opt-in E2E env switches are enabled, `GAGENT_E2E_DEPS` points at this checkout's `backend/temp/e2e_deps`, that dependency directory contains the `agents`, `browser_use`, and `playwright` import markers plus pinned `.dist-info` directories from `backend/requirements-e2e.txt`, and score input reports exist with non-empty contents;
